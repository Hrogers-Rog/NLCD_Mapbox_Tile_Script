...
def world_tile_bounds(profile: MapProfile, tile_x: int, tile_y: int) -> GeographicBounds:
    """Convert one signed Railroader terrain tile into geographic bounds."""
    tile_size = profile.tile_dimension_meters
    cosine = math.cos(math.radians(profile.origin_latitude))
    if abs(cosine) < 1e-12:
        raise MapProfileError("The profile origin is too close to a pole for local projection.")

    west_meters = tile_x * tile_size + profile.origin_east_bias_meters
    east_meters = (tile_x + 1) * tile_size + profile.origin_east_bias_meters
    south_meters = tile_y * tile_size + profile.origin_north_bias_meters
    north_meters = (tile_y + 1) * tile_size + profile.origin_north_bias_meters
    longitude_scale = 111111.0 * cosine
    return GeographicBounds(
        west=profile.origin_longitude + west_meters / longitude_scale,
        south=profile.origin_latitude + south_meters / 111111.0,
        east=profile.origin_longitude + east_meters / longitude_scale,
        north=profile.origin_latitude + north_meters / 111111.0,
    )


def load_world_tile_list(path: Path, expected_profile_id: str) -> list[tuple[int, int]]:
    try:
        document = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise MapProfileError(f"Could not read route tile list {path}: {exc}") from exc

    if not isinstance(document, dict) or not isinstance(document.get("tiles"), list):
        raise MapProfileError(f"Route tile list {path} must contain a tiles array.")
    profile_id = document.get("profile")
    if profile_id and profile_id != expected_profile_id:
        raise MapProfileError(
            f"Route tile list {path} targets profile '{profile_id}', not '{expected_profile_id}'."
        )

    selected = set()
    for index, tile in enumerate(document["tiles"]):
        if not isinstance(tile, dict):
            raise MapProfileError(f"Route tile list {path} tiles[{index}] must be an object.")
        tile_x = tile.get("x")
        tile_y = tile.get("y")
        if (
            ...
            raise MapProfileError(
                f"Route tile list {path} tiles[{index}] must contain integer x and y values."
            )
        selected.add((tile_x, tile_y))
    if not selected:
        raise MapProfileError(f"Route tile list {path} does not select any terrain tiles.")
    return sorted(selected)


def source_windows_for_world_tiles(
    profile: MapProfile,
    world_tiles: list[tuple[int, int]],
    zoom: int,
    tile_size: int,
) -> tuple[GeographicBounds, dict[tuple[int, int], list[GeographicBounds]]]:
    windows: dict[tuple[int, int], list[GeographicBounds]] = {}
    geographic_tiles = [world_tile_bounds(profile, x, y) for x, y in world_tiles]
    overall = GeographicBounds(
        west=min(bounds.west for bounds in geographic_tiles),
        south=min(bounds.south for bounds in geographic_tiles),
        east=max(bounds.east for bounds in geographic_tiles),
        north=max(bounds.north for bounds in geographic_tiles),
    )
    for bounds in geographic_tiles:
        for source_x, source_y in source_tiles(bounds, zoom, tile_size):
            windows.setdefault((source_x, source_y), []).append(bounds)
...
def scan_one_tile_windows(
    cache: TerrainRgbCache,
    windows: list[GeographicBounds],
    zoom: int,
    tile_x: int,
    tile_y: int,
    tile_size: int,
    ...
    source = cache.get_tile(zoom, tile_x, tile_y, refresh=refresh_cache)
    heights = decode_terrain_rgb(source.image)
    selected = np.zeros(heights.shape, dtype=bool)
    for bounds in windows:
        x0, x1, y0, y1 = pixel_window_for_tile(bounds, zoom, tile_x, tile_y, tile_size)
        if x0 < x1 and y0 < y1:
            selected[y0:y1, x0:x1] = True

    finite = np.isfinite(heights) & selected
    if not np.any(finite):
        return None
    ...
    max_pixel_y, max_pixel_x = maximum_index
    min_lon, min_lat = pixel_center_to_lonlat(
        zoom, tile_x, tile_y, min_pixel_x, min_pixel_y, tile_size
    )
    max_lon, max_lat = pixel_center_to_lonlat(
        zoom, tile_x, tile_y, max_pixel_x, max_pixel_y, tile_size
    )
    return TileScanResult(
        minimum_meters=float(heights[minimum_index]),
        minimum_longitude=min_lon,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Scan only the signed Railroader tiles in a route-footprint JSON file "
            "instead of the profile's rectangular demScan bounds."
        ),
    )
    ...
    if args.tile_list is not None:
        ...
        try:
            world_tiles = load_world_tile_list(tile_list_path, profile.profile_id)
            bounds, source_windows = source_windows_for_world_tiles(
                profile, world_tiles, zoom, tile_size
            )
        except MapProfileError as exc:
            parser.error(str(exc))
        tiles = sorted(source_windows)
        footprint = {
            "kind": "railroaderTileList",
            "path": str(tile_list_path),
            "railroaderTileCount": len(world_tiles),
            "bounds": {
                "westLongitude": bounds.west,
                "southLatitude": bounds.south,
                "eastLongitude": bounds.east,
    ...
    footprint_label = "sparse route tiles" if source_windows is not None else "rectangle"
    print(
        f"Footprint ({footprint_label}): {bounds.west:.6f},{bounds.south:.6f} to "
        f"{bounds.east:.6f},{bounds.north:.6f}"
    )
    if footprint is not None:
        print(f"Railroader terrain tiles: {footprint['railroaderTileCount']:,}")
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.workers)) as executor:
        if source_windows is None:
            ...
            futures = {
                executor.submit(
                    scan_one_tile_windows,
                    cache,
                    source_windows[(tile_x, tile_y)],
                    zoom,
                    tile_x,
                    tile_y,
                    tile_size,
